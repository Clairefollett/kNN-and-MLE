...
def main():
    ...
    for k in [1,3,5,7,9,99,999,8000]:
        ...
        # Predict on the entire training set using the current k
...
def get_nearest_neighbors(example_set, query, k):
    # Subtract each row in example_set by query vector
    row_vector_substraction = example_set - query

    # Calculate the distances (by calculating the norms)
    distances = np.linalg.norm(row_vector_substraction, axis=1)

    # Get k number of indices of nearest points
...
def knn_classify_point(examples_X, examples_y, query, k):
    # Call get_nearest_neighbors and stored in idx_of_nearest
    idx_of_nearest = get_nearest_neighbors(examples_X, query, k)

    # Get the labels of the nearest neighbors
    nearest_labels = examples_y[idx_of_nearest]

    # Count how many 1's there are
    non_zero_vals = np.count_nonzero(nearest_labels)

    # If the majority is 1, set that to predicted_label
    # Else, predicted_label is 0
    if non_zero_vals > k/2:
        predicted_label = 1
    else:
        predicted_label = 0
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
    chunks_X = np.split(train_X, num_folds)
    A_X, B_X, C_X, D_X = chunks_X

    chunks_y = np.split(train_y, num_folds)
    A_y, B_y, C_y, D_y = chunks_y

    accuracies = []

    # Predict A: B + C + D
    # Use A, B, C as traiing set
    # A as validation set
    train_A_X = np.vstack((B_X, C_X, D_X))
    train_A_y = np.vstack((B_y, C_y, D_y))
    result_A = predict(train_A_X, train_A_y, A_X, k)
    accuracy_A = compute_accuracy(A_y, result_A)
    accuracies.append(accuracy_A)
    
    # Predict B: A + C + D
    train_B_X = np.vstack((A_X, C_X, D_X))
    train_B_y = np.vstack((A_y, C_y, D_y))
    result_B = predict(train_B_X, train_B_y, B_X, k)
    accuracy_B = compute_accuracy(B_y, result_B)
    accuracies.append(accuracy_B)
    # Predict C: A + B + D
    train_C_X = np.vstack((A_X, B_X, D_X))
    train_C_y = np.vstack((A_y, B_y, D_y))
    result_C = predict(train_C_X, train_C_y, C_X, k)
    accuracy_C = compute_accuracy(C_y, result_C)
    accuracies.append(accuracy_C)
    # Predict D: A + B + C
    train_D_X = np.vstack((A_X, B_X, C_X))
    train_D_y = np.vstack((A_y, B_y, C_y))
    result_D = predict(train_D_X, train_D_y, D_X, k)
    accuracy_D = compute_accuracy(D_y, result_D)
